...
/**
 * The TiledTile class represents a tile in a Tiled tile set. A tile has an
 * integer ID, an (optional) animation and supports custom properties.
 *
 * @since 0.1.0
 */
class TiledTile final {
 public:
   ...
   * @since 0.1.0
   */
  [[nodiscard]] const TiledAnimation& get_animation() const noexcept;

  [[nodiscard]] bool has_attribute(const std::string& name) const noexcept;

  /**
   * Indicates whether or not the tile has a property with the specified name.
   *
   * @param name the name of the property to look for.
   * @return true if a property with the specified name is found; false
   * otherwise.
   * @since 0.1.0
   */
  [[nodiscard]] bool has_property(const std::string& name) const noexcept;
   ...
   * This method might return unexpected values if the property doesn't
   ...
   * @since 0.1.0
   */
  [[nodiscard]] int get_int_property(const std::string& name) const;

  /**
   * Returns the value of the property with the specified name as a float. This
   * method might return unexpected values if the property doesn't represent a
   * float.
   *
   ...
   * @since 0.1.0
   */
  [[nodiscard]] float get_float_property(const std::string& name) const;

  /**
   * Returns the value of the property with the specified name as a bool. This
   * method might return unexpected values if the property doesn't represent a
   * bool. This method expects the property value to be in all lower case, more
   * specifically, the method checks whether or not the property value is equal
   * to the string "true".
   *
   ...
   * @since 0.1.0
   */
  [[nodiscard]] bool get_bool_property(const std::string& name) const;
   ...
   * @since 0.1.0
   */
  [[nodiscard]] const std::string& get_string_property(
      const std::string& name) const;

  [[nodiscard]] int get_int_attribute(const std::string& name) const;

  [[nodiscard]] std::string get_string_attribute(const std::string& name) const;
   ...
   * @since 0.1.0
   */
  [[nodiscard]] const TiledObject& get_object(const std::string& name) const;
 ...
 private:
  pugi::xml_node m_tileNode;
  TiledAnimation m_animation;
   ...
   * @since 0.1.0
   */
  [[nodiscard]] const TiledProperty& get_property(
      const std::string& name) const;
};
