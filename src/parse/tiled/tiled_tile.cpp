...
{
  for (const auto animationNode : m_tileNode.children("animation")) {
    auto index = 0;
    for (const auto frameNode : animationNode.children("frame")) {
      const auto frameId =
          firstTileSetId + frameNode.attribute("tileid").as_int();
      const auto duration = frameNode.attribute("duration").as_int();

      m_animation.add_frame(Frame{frameId, duration});
      ++index;
    }
  }

  m_animated = !m_animation.get_frames().empty();
}

void TiledTile::process_properties()
{
  const auto propertiesNode = m_tileNode.child("properties");
  for (const auto propertyNode : propertiesNode.children("property")) {
    ...
    m_properties.push_back(TiledProperty{name, type, value});
  }
}

void TiledTile::process_object_groups()
{
  const auto objectGroupNode = m_tileNode.child("objectgroup");
...
{
  return !m_tileNode.attribute(name.c_str()).empty();
}
...
bool TiledTile::has_object(const std::string& name) const noexcept
{
  for (const auto& object : m_objects) {
    if (object.get_attribute("name") ==
        name) {  // may theoretically throw, but shouldn't
      return true;
    }
  }
...
const TiledProperty& TiledTile::get_property(const std::string& name) const
{
  ...
  const auto msg =
      "Failed to find property: " + name + ", in tile " + std::to_string(m_id);
...
{
  return m_tileNode.attribute(name.c_str()).as_int();
}

std::string TiledTile::get_string_attribute(const std::string& name) const
{
  return m_tileNode.attribute(name.c_str()).as_string();
}
...
const TiledObject& TiledTile::get_object(const std::string& name) const
{
  ...
  const auto msg = "Failed to find object with name: " + name + ", in tile " +
                   std::to_string(m_id);
  throw std::invalid_argument(msg);
}
