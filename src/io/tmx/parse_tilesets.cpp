...
{
  auto id = static_cast<TileID>(tileset.first_id.get());
...
{
  const auto first = static_cast<TileID>(tileset.first_id.get());
  for (const auto& tsTile : tileset.tiles) {
    const auto gid = first + static_cast<TileID>(tsTile.id.get());
