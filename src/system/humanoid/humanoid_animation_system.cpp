...
{
  registry.view<comp::humanoid>().each([&](entt::entity e) {
    if (registry.has<comp::humanoid_move>(e)) {
      update_move_animation(registry, e);
    }
    if (registry.has<comp::humanoid_attack>(e)) {
      update_attack_animation(registry, e);
    }
  });
}
