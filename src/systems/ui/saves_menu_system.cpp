...
auto create_saves_menu(entt::registry& registry) -> comp::menu::entity
{
  ...
  menu.title = "Saves";
  menu.blocking = true;
  menu.buttons = make_buttons(registry);
  menu.binds = make_binds(registry);

  auto& drawable = registry.emplace<comp::menu_drawable>(menuEntity);
  drawable.lines = make_lines(registry);
  drawable.labels = make_labels(registry);

  return comp::menu::entity{menuEntity};
}
