...
void menu_button::calculate_bounds(cen::renderer& renderer) const
{
  ...
  const auto calcX = [&]() -> int {
    ...
    if (m_col == -1) {
      ...
      return (m_col * glob::menuColSize) - (width / 2);
    }
  };

  const auto y = static_cast<float>(m_row * glob::menuRowSize) -
...
void menu_button::load_normal_text(cen::renderer& renderer) const
{
  ...
  m_texture.emplace(
      to_texture(renderer, m_text, renderer.get_font(glob::menuSmallFont)));
}
