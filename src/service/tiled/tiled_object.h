...
class TiledObject final {
 ...
 public:
  ...
  [[nodiscard]] const TiledProperty& GetProperty(const std::string& id) const;

  [[nodiscard]] bool HasProperty(const std::string& id) const noexcept;

  [[nodiscard]] const std::vector<TiledProperty>& GetProperties() const noexcept;
};
